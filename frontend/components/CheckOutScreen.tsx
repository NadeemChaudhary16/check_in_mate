...
export default function CheckOutScreen({ navigation }: { navigation: any }) {
  ...
  const handleCheckOut = async () => {
    ...
    try {
      ...
      // Send the base64Image to the backend for check-out
      const response = await axios.post(`${BASE_URL}/api/check-out`, {
      ...
      if (response.data.success) {
        ...
        if (response.data.message.includes("already checked out")) {
          Alert.alert("Error", response.data.message);
          console.log("Visitor has already checked out.");
        } else {
